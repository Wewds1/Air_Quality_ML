...
function renderTimeline(timeline) {
  ...
  elements.timelineShell.innerHTML = `
    <svg viewBox="0 0 ${width} ${height + 30}" preserveAspectRatio="none" aria-label="AQI and risk timeline">
      ...
      <path d="${alertPath}" fill="none" stroke="#8a8a8a" stroke-width="2" stroke-dasharray="4 7" />
    </svg>
    <div class="timeline-meta">
      <div>
        <span class="metric-label">Latest Avg AQI</span>
        <strong>${formatCompact(lastPoint.avg_aqi, 1)}</strong>
      </div>
      <div>
        <span class="metric-label">Peak Probability</span>
        <strong>${formatPercent(lastPoint.max_risk_prob)}</strong>
      </div>
      <div>
        <span class="metric-label">Active Rows</span>
